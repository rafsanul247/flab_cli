...
void main() {
  group('FLAB Feature Scaffolding & Templates', () {
    ...
    test('Templates.getUseCaseContent generates the use case template', () {
      ...
      expect(
          content, contains("import '../../../../core/error/failures.dart';"));
      expect(content, contains("import '../entities/home_entity.dart';"));
      expect(
          content, contains("import '../repositories/home_repository.dart';"));
      expect(content, contains('class HomeUseCase'));
      expect(content, contains('final HomeRepository repository;'));
      expect(
          content,
          contains(
              'Future<Either<Failure, List<HomeEntity>>> getUsers() async'));
    });
    ...
    test('Templates generate selected state management and MVC files', () {
      ...
      final provider =
          Templates.getStateManagementContent('Home', 'home', 'provider');
      final riverpod =
          Templates.getStateManagementContent('Home', 'home', 'riverpod');
    ...
    test('flab creates entities template and widgets directory', () async {
      ...
      try {
        ...
        final binPath = path.canonicalize(
            path.join(Directory.current.path, 'bin', 'flab.dart'));
        ...
        final entityFile = File(
          path.join(tempDir.path, 'lib', 'features', 'home', 'domain',
        ...
        final modelFile = File(
          path.join(tempDir.path, 'lib', 'features', 'home', 'data', 'models',
              'home_model.dart'),
        );
        expect(modelFile.existsSync(), isTrue);
        expect(modelFile.readAsStringSync(),
            contains('class HomeModel extends HomeEntity'));

        // Verify data/data_sources/home_data_source.dart exists and has updated template
        final dataSourceFile = File(
          path.join(tempDir.path, 'lib', 'features', 'home', 'data',
              'data_sources', 'home_data_source.dart'),
        );
        ...
        expect(
            ...
            contains(
                'class HomeDataSourceImplement implements HomeDataSource'));
        expect(dataSourceFile.readAsStringSync(),
            contains("import '../models/home_model.dart';"));

        // Verify presentation/views/widgets directory exists
        final widgetsDir = Directory(
          path.join(tempDir.path, 'lib', 'features', 'home', 'presentation',
              'views', 'widgets'),
        );
