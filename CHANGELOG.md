...
- Fixed Clean Architecture state-management scaffolding so GetX uses the manager controller without generating a duplicate state controller.
- Prevented non-GetX Clean Architecture features from registering or importing a GetX controller.
- Kept Provider, Riverpod, and Bloc starter state files available when selected.
- Added tests covering the GetX controller generation behavior.

## [1.0.3] - 2026-09-05

- Fixed data and domain boilerplates, including UseCase templates.
- Fixed MVC and MVVM feature structures.
- Added state-management selection for Clean Architecture, MVC, and MVVM.
- Updated CLI command prompt and state-management dependencies.
- Updated package and CLI version to `1.0.3`.

## 1.0.2

- Fixed unused imports in `example.dart`.
- Updated core dependencies to the latest versions.
- Fixed `widget_test.dart` initialization errors.
